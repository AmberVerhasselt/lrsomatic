...
- [#196](https://github.com/IntGenomicsLab/lrsomatic/pull/196) - `LRSOMATICREPORT` now runs [lrsomatic_report](https://github.com/ljwharbers/lrsomatic_report) v1.5.0 from `ghcr.io/ljwharbers/lrsomatic-report:1.5.0` (`oras://ghcr.io/ljwharbers/lrsomatic-report-sif:1.5.0` under Singularity/Apptainer) instead of a copy of the tool's source vendored at `assets/lrsomatic_report`, which is deleted. Updating the report tool is now a container tag bump rather than a re-sync of ~9,900 lines plus a version string in seven places, and the version in `versions.yml` is read from `render_report.R --version` instead of being hardcoded. The image is `linux/amd64` only and **Conda is not supported for this step** — the module errors under `-profile conda`/`mamba`, so `conda` is dropped from the nf-test CI matrix until `lrsomatic-report` reaches bioconda; use `--skip_report` if you need a conda run. Rendered reports are unchanged (@ljwharbers).
...
- [#195](https://github.com/IntGenomicsLab/lrsomatic/pull/195) - Re-synced the vendored [lrsomatic_report](https://github.com/ljwharbers/lrsomatic_report) to v1.4.0. The small-variant table now reads the VEP plugin annotations #193 added and the report previously threw away: AlphaMissense (`am_class`, `am_score`), ClinVar (`clinvar`, `clinvar_id`, linked to the ClinVar record), CADD, REVEL and EVE, plus SIFT and PolyPhen from the `PolyPhen_SIFT` plugin on CHM13, whose cache carries neither and where both columns were empty on every report. Each predictor's class and score are separate columns, so the class gets a tickbox filter and the score sorts numerically, and pathogenic-class calls are tinted. A column appears only when the annotated VCF declared that field, so a plugin that was not on gives no column rather than an empty one, `cosmic` and `dbsnp` are absent on CHM13, and a new "Annotation sources" footnote names what was present and what was not. The release also fixes the dbSNP/COSMIC and `Existing_variation` handling on the VEP text output path, draws the circos y-axes on the tracks they label, gives the SV palette and y-positions one definition, guards the CNV axis, and drops the panel hero cards. No container change: the tool's dependencies are unchanged (@ljwharbers).
