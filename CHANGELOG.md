...
- [#197](https://github.com/IntGenomicsLab/lrsomatic/pull/197) - Added CHM13 support for ClairS-TO's Verdict module, which tags tumour-only calls as germline, somatic or subclonal somatic. Verdict's ASCAT loci, allele and GC content resources were GRCh38-only, so on CHM13 germline variants went untagged and leaked into `somatic.vcf.gz`. With `--genome CHM13` the pipeline now lays out a CHM13 resource set from the ASCAT files it already downloads and passes it to ClairS-TO as `--cna_resource_dir`. Correction of LogR is GC-only: no replication timing file is published for CHM13, and lifting the GRCh38 one over leaves purity identical, ploidy within 0.001 and 0.002% of variants differently tagged, while being poorly anchored across the acrocentric contigs. A prepared directory can be supplied instead with `--clairsto_cna_resources` — including one carrying an `RT_<name>.txt` — and is checked at startup for the layout ClairS-TO expects and for links reaching outside it, which would be missing inside the container. If the loci cannot belong to the reference, ClairS-TO disables Verdict with a warning rather than applying the wrong coordinates (@ljwharbers).
...
- [#197](https://github.com/IntGenomicsLab/lrsomatic/pull/197) - `CLAIRSTO` now runs `ghcr.io/ljwharbers/clairs-to:0.5.1-verdict-chm13-e2987bf` (ClairS-TO 0.5.1) instead of `docker.io/hkubal/clairs-to:v0.4.2`, the fork that lets Verdict resolve its CNA resources from `--cna_resource_dir`. **GRCh38 results move as well as CHM13 ones**: besides the 0.4.2 to 0.5.1 upgrade, the fork fixes Verdict's GC window selection, which read the 25bp window's correlations with the other windows rather than with LogR, so the correction window was chosen by the wrong quantity. The image also makes its bundled GRCh38 resources world-readable and disables Verdict with a warning when a resource set exists but cannot be read, instead of failing inside `alleleCounter`: they ship mode 0640, which a container run under `-u $(id -u):$(id -g)` cannot open. Revert to the upstream image once HKU-BAL/ClairS-TO carries these changes. The module also passes `--conda_prefix` and selects the SIF under `-profile apptainer`, not only `-profile singularity`, and no longer passes `--sample_name` twice (@ljwharbers).
