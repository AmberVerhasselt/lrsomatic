...
- [#196](https://github.com/IntGenomicsLab/lrsomatic/pull/196) - Rebuilt the builtin `lymphoid` panel and scoped its genes per table, re-synced from lrsomatic_report v1.6.0. `assets/gene_lists/lymphoid.{hg38,t2t}.tsv` now carry that release's optional `applies_to` column — blank filters both variant tables, `snv` the small-variant table only, `sv` the SV table only, and any other value is a hard error — and merge two curated NHL lists: 128 genes with per-reference spans from `NHL_Genes_{T2T,hg38}.bed` as the SV set, 149 symbols from `twist_genes.tsv` as the coding-mutation set, 42 of them on both, giving 235 rows against the previous 72 genes. Coordinates are required only on rows that can match an SV, so the `snv`-only rows carry none. What this changes for a reader ticking `lymphoid`: the rearrangement partners (`IGH`, `IGK`, `IGL`, `TRA/D`, `TRB`, `TRG`, `DUSP22`) are in the SV table for the first time, `MYD88` and `NOTCH1` and 24 other coding genes now filter only the small-variant table, `BCL10`, `BCL11A`, `BCL6`, `HLA-A`, `HLA-B`, `MALT1` and `SYK` only the SV table, and 18 genes of the old list (among them `CD19`, `MS4A1`, `SOX11`, `FAT1`, `KLHL6`, `SPEN`) are not in the new lists at all. `sarcoma` is untouched and, carrying no `applies_to` column, still filters both tables (@ljwharbers).
