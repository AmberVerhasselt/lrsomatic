...
- [#196](https://github.com/IntGenomicsLab/lrsomatic/pull/196) - The report tool's v1.4.0 changes reach the pipeline with the v1.5.0 container. The small-variant table now reads the VEP plugin annotations #193 added and the report previously threw away: AlphaMissense (`am_class`, `am_score`), ClinVar (`clinvar`, `clinvar_id`, linked to the ClinVar record), CADD, REVEL and EVE, plus SIFT and PolyPhen from the `PolyPhen_SIFT` plugin on CHM13, whose cache carries neither and where both columns were empty on every report. Each predictor's class and score are separate columns, so the class gets a tickbox filter and the score sorts numerically, and pathogenic-class calls are tinted. A column appears only when the annotated VCF declared that field, so a plugin that was not on gives no column rather than an empty one, `cosmic` and `dbsnp` are absent on CHM13, and a new "Annotation sources" footnote names what was present and what was not. The release also fixes the dbSNP/COSMIC and `Existing_variation` handling on the VEP text output path, draws the circos y-axes on the tracks they label, gives the SV palette and y-positions one definition, guards the CNV axis, and drops the panel hero cards (@ljwharbers).
