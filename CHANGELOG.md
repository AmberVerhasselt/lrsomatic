...
- [#201](https://github.com/IntGenomicsLab/lrsomatic/pull/201) - `CLAIRSTO` and `CLAIRSTO_VERDICT_TAG` now run `ghcr.io/ljwharbers/clairs-to:0.5.1-verdict-chm13-c0687e8-flat` as a `docker://` image under Singularity and Apptainer as well, so the SIF is built locally on first use instead of being pulled as an `oras://` SIF. The `-cpu` SIF still failed with `PROTOCOL_ERROR`: the signed blob URL ghcr redirects to expires on the next quarter-hour boundary, which can be minutes away, and Apptainer fetches an `oras://` SIF as one stream with no retry and no resume. A `docker://` pull goes through containers/image, which resumes a cut layer with a Range request and a fresh URL. `-flat` is the same software copied into an empty image in a few layers (3.3 GB instead of 7 GB of base-image layers). The first run needs about 10 GB of Apptainer temporary space and a few minutes for the conversion; `docs/usage.md` describes that, `pullTimeout`, pre-pulling, and how to recover any `oras://` SIF resumably (@ljwharbers).
