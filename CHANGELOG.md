...
- [#197](https://github.com/IntGenomicsLab/lrsomatic/pull/197) - `CLAIRSTO` now runs `ghcr.io/ljwharbers/clairs-to:0.5.1-verdict-chm13` (ClairS-TO 0.5.1) instead of `docker.io/hkubal/clairs-to:v0.4.2`, the fork that lets Verdict resolve its CNA resources from `--cna_resource_dir`. **GRCh38 results move as well as CHM13 ones**: besides the 0.4.2 to 0.5.1 upgrade, the fork fixes Verdict's GC window selection, which read the 25bp window's correlations with the other windows rather than with LogR, so the correction window was chosen by the wrong quantity. Revert to the upstream image once HKU-BAL/ClairS-TO carries these changes. The module also passes `--conda_prefix` and selects the SIF under `-profile apptainer`, not only `-profile singularity`, and no longer passes `--sample_name` twice (@ljwharbers).
